...
const Important: FC<ImportantProps> = (props) => {
	...
	return (
		<span className={`important-text ${props?.className ?? ''}`} data-important-weight={weight}>
			{props.children}
		</span>
	);

	// if (Array.isArray(props.children) && props.children.length > 0) {
	// 	return <span className={`important-text ${props?.className ?? ''}`} data-important-weight={weight}>
	// 		{props.children}
	// 	</span>
	// 	return <span className={`important-text ${props?.className ?? ''}`} data-important-weight={weight}>
	// 		{props.children}
